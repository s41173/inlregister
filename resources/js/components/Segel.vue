          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Segel List</h3>
                ...
                <table class="table table-hover">
                  ...
                  <tbody>
                     <tr v-for="seg in segels.data" :key="seg.id">
                      ...
                      <td>{{ (seg.terpakai == 1 ? "used" : "not yet") }}</td>
                      <td v-if='seg.terpakai != "1"'>
                ...
                <form @submit.prevent="editmode ? updateUser() : createUser()">
                    <div class="modal-body">
                        <div class="form-group">
                            <label>Number</label>
                            <input v-model="form.nomor" type="text" name="nomor"
                                class="form-control" :class="{ 'is-invalid': form.errors.has('nomor') }">
                            <has-error :form="form" field="nomor"></has-error>
                        </div>
                        <div class="form-group">
                            <label>Description</label>
                            <input v-model="form.keterangan" type="text" name="keterangan"
        ...
        methods: {
          ...
          loadSegels(){
            // if(this.$gate.isAdmin()){
              axios.get("api/segel").then(({ data }) => (this.segels = data.data));
            // }
          },
        updateUser(){
                this.$Progress.start();
                // console.log('Editing data');
                this.form.put('api/segel/'+this.form.id)
                .then((response) => {
                    // success
                    $('#addNew').modal('hide');
                    Toast.fire({
                ...
                Swal.fire({
                    ...
                    confirmButtonText: 'Yes, delete it!'
                        ...
                        // Send request to the server
                         if (result.value) {
                                this.form.delete('api/segel/'+id).then(()=>{
                                        Swal.fire(
                                        'Deleted!',
